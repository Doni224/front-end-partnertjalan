<template>
  <div>
    <div class="bg">
      <img
        class="hero-bg"
        src="../assets/img/gbr-utama.png"
        alt="Background"
        width="50%"
      />
    </div>
    <div class="hero container">
      <div class="hero-dk h-100">
        <div class="row mt-4">
          <div class="col-md-12">
            <div class="d-flex heroku" style="height: 70vh">
              <div>
                <h1 class="mb-4 hero-title">
                  Wisata terbaik <br />
                  untuk pengalaman <br />
                  terbaik
                </h1>
                <p class="mb-4 subheading">
                  Yuk pilih wisata yang kamu inginkan dan <br />
                  rasakan perjalanan menyenangkan yang <br />
                  ga pernah kamu lupakan.
                </p>
                <button class="btn btn-md btn-primary mt-2">
                  Mulai Sekarang
                </button>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div class="hero-mb h-100">
        <div class="row mt-4">
          <div class="col-md-6">
            <div class="d-flex align-items-center mb-5" style="height: 70vh">
              <div>
                <h2 class="mb-3 title">
                  Wisata terbaik untuk <br />pengalaman yang terbaik.
                </h2>
                <p class="mb-4 heading">
                  Yuk pilih wisata yang kamu inginkan dan <br />
                  rasakan perjalanan menyenangkan yang <br />
                  ga pernah kamu lupakan.
                </p>
                <b-button href="#app1" class="btn btn-md btn-primary mt-2">
                  Mulai Sekarang
                </b-button>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
    <ContohWisata />
    <PaketTerbaru/>
    <DaftarWisata/>
    <PartnertshipApp />
    <WhyusApp />
    <TestimoniApp/>
    <KontakApp/>
    <div class="foter mt-5">
      <FooterApp/>
    </div>
    
  </div>
</template>

<script>
import WhyusApp from "./WhyusApp.vue";
import PartnertshipApp from "./PartnertshipApp.vue";
import ContohWisata from "./ContohWisata.vue";
import KontakApp from "./KontakApp.vue";
import FooterApp from "./FooterApp.vue";
import DaftarWisata from "./DaftarWisata.vue";
import PaketTerbaru from "./PaketTerbaru.vue";
import TestimoniApp from "./TestimoniApp.vue";


export default {
  name: "HeaderApp",
  components: { WhyusApp, DaftarWisata, PartnertshipApp, ContohWisata, KontakApp, FooterApp, PaketTerbaru, TestimoniApp },
};
</script>

<style>
  * {
    scroll-behavior: smooth;
}
.bg {
  position: absolute;
  top: 0;
  left: 0;
  width: 100%;
  height: 100vh;
  overflow: hidden;
  z-index: -9999;
}
.hero-title {
  font-size: 42px;
  color: #333;
}
.hero-bg {
  position: relative;
  top: 0;
  left: 0;
  float: right;
  z-index: -9999;
}
.hero-mb {
  display: none;
}
.cta {
  position: relative;
  float: right;
  bottom: 85%;
  right: 2.5%;
}

.cta .btn {
  background: #7cb1a1;
  padding: 2rem;
  font-size: 18px;
  color: #fff;
  border: none;
}

.cta .btn:hover {
  background: #3e6f7c;
  color: #fff;
}
@media only screen and (max-width: 800px) {
  .hero-title {
    font-family: "Lato", sans-serif;
    font-style: normal;
    font-weight: 700;
    font-size: 35px;
    line-height: 43px;
  }
  .subheading {
    font-family: "Roboto", sans-serif;
    font-style: normal;
    font-weight: 400;
    font-size: 18px;
    line-height: 150%;
    margin-top: 40px;
  }
}

@media only screen and (max-width: 380px) {
  .bg {
    height: 100%;
  }
  .hero-bg {
    top: 0;
    left: 0;
    z-index: -9999;
    width: 135%;
    height: 190%;
  }
  .hero-mb {
    display: block;
    margin-bottom: 97%;
  }
  .hero-dk {
    display: none;
  }
  .form-label {
    font-weight: bold;
    opacity: 0.7;
  }
  .title {
    color: #0e0f0f;
    font-size: 36px;
  }
  .heading {
    color: #0e0f0f;
    font-family: "Roboto", sans-serif;
    font-style: normal;
    font-weight: 400;
    font-size: 16px;
    line-height: 150%;
  }
}
</style>